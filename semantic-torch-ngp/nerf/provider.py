...
class NeRFDataset:
    def __init__(self, opt, device, type='train', downscale=1, n_test=10):
        ...
        if self.semantics is not None:
            self.semantics= torch.from_numpy(np.stack(self.semantics, axis=0)) # [N, H, W, C]
    ...
    def collate(self, index):
        ...
        # NOTE: this is where images are changed into a weird format
        # TODO: figure out what this does 
        if self.images is not None:
            images = self.images[index].to(self.device) # [B, H, W, 3/4]
            if self.training:
                # C means channels (RGB has 3)
                C = images.shape[-1]
                images = torch.gather(images.view(B, -1, C), 1, torch.stack(C * [rays['inds']], -1)) # [B, N, 3/4]
            results['images'] = images

        # its probably fine to have semantics do the same thing as RGB
        if self.semantics is not None:
