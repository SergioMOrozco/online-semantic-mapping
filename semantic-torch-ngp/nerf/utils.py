...
class Trainer(object):
    def __init__(self, 
        ...
        if isinstance(semantic_criterion, nn.Module):
            semantic_criterion.to(self.device)
    ...
    def train_step(self, data):
        ...
        # These rays just kind of look like empty rays that we shoot per image. They tell use where the rays go for our input images
        ...
        #pred_semantic = torch.tensor(pred_semantic)
    ...
    def test_gui(self, pose, intrinsics, W, H, bg_color=None, spp=1, downscale=1):
        ...
        with torch.no_grad():
            with torch.cuda.amp.autocast(enabled=self.fp16):
                ...
                preds, preds_depth, preds_semantic = self.test_step(data, bg_color=bg_color, perturb=False if spp == 1 else spp)
