...
class NeRFNetwork(NeRFRenderer):
    ...
    def color(self, x, d, mask=None, geo_feat=None, **kwargs):
        ...
        #print("RGB NETWORK OUTPUT SHAPE")
        #print(rgbs.shape)

        return rgbs        

    def semantic(self, x, d, mask=None, geo_feat=None, **kwargs):
        # x: [N, 3] in [-bound, bound]
        # mask: [N,], bool, indicates where we actually needs to compute rgb.

        if mask is not None:
            semantic = torch.zeros(mask.shape[0], self.num_semantic_classes, dtype=x.dtype, device=x.device) # [N, 3]
            # in case of empty mask
            if not mask.any():
                return semantic 
            x = x[mask]
        ...
        if mask is not None:
            semantic[mask] = h.to(semantic.dtype) # fp16 --> fp32
        else:
            semantic = h

        # print("SEMANTIC NETWORK OUTPUT SHAPE")
